...
pool:
	...
	filepath[3] = ('0' + (filecount % 9));
	fp_open = fopen(filepath, "rb+");
	if (!feof(fp_open)) {
		//std::cout << "00";
		int true_size = fread(buff, 1, buf_size, fp_open);
		while (i < 32768) {
			buf[i] = buff[i];
			i++;
		}
		if (buf[3] == 0x67) { std::cout << "good"; }
		std::cout << sizeof(buf[3]);
		filecount += 1;
		fclose(fp_open);
		//std::cout << true_size;
		return true_size;
		...
		if ((unsigned char)recvbuf[4] == 0xd1 && (unsigned char)recvbuf[5] == 0x01) {
			...
			while (i<(((unsigned char)recvbuf[2] - 0x00) * 16 * 16 + ((unsigned char)recvbuf[3] - 0x00)) + 4) {
				...
				while (bytecount ==32768) {
					...
					bytecount = 0;
					filepath2[3] = ('0' + (filescount % 9));
					fopen_s(&FSPOINTER, filepath2, "wb+");
					//nu = 0;
					nu = (unsigned char)recvbuf[i] / 1;  //unsigned char 转换成 int
					fwrite(&nu, 1, 1, FSPOINTER);
					goto ii;
					//break;
					
					//fopen_s(&FSPOINTER, filetxt, "a+");
				}
				nu = (unsigned char)recvbuf[i] / 1;  
				fwrite(&nu, 1, 1, FSPOINTER);
